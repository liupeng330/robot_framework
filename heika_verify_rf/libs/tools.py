...
from request_utils.utils import *
...
def user_login(username):
    helper.log("使用用户名'%s'登陆" % username)
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == 'init':
        for user_id in sys.argv[2:]:
            ...
            helper.log('将user_id为%s的用户置为待调查状态' % user_id)
            init_user(user_id)

            helper.log('删除审核状态表原有数据')
            delete_verify_user_status_by_user_id(user_id)

            helper.log('调用接口模拟初始化审核状态')
            helper.log(request_util.init_user_from_mobile(user_id))

            helper.log('调用接口模拟提交审核')
            helper.log(request_util.commit_user_from_mobile(user_id))
        sys.exit(0)

    if sys.argv[1] == 'cleanup_by_user_id':
        for user_id in sys.argv[2:]:
            helper.log('将user_id为%s的用户任务清除' % user_id)
            verify_user_status_id = get_verify_user_status_id_by_user_id(user_id)

            if verify_user_status_id is None:
                helper.log_error('未找到user_id=%s对应的verify_user_status数据' % user_id)
                pass

            helper.log('verify_user_status_id = %s' % verify_user_status_id)
            helper.log('删除verify_process_task表中的数据, verify_user_status_id为%s' % verify_user_status_id)
    ...
    if sys.argv[1] == 'cleanup_by_executor_name':
        for real_name in sys.argv[2:]:
            helper.log('将real_name为%s的用户任务清除' % real_name)
            verify_user_id = get_verify_user_id_by_real_name(real_name)

            if verify_user_id is None:
                helper.log_error('未找到real_name=%s的verify_user数据' % real_name)
                pass

            helper.log('verify_user_id = %s' % verify_user_id)
            helper.log('删除verify_process_task表中的数据, executor为%s' % verify_user_id)
    ...
    if sys.argv[1] == 'inv_pass':
        ...
        for user_id in sys.argv[3:]:
            helper.log('将user_id为%s的用户通过调查' % user_id)
    ...
    if sys.argv[1] == 'first_verify_pass':
        request_util = user_login(sys.argv[2])
        for user_id in sys.argv[3:]:
            helper.log('将user_id为%s的用户通过一审' % user_id)
    ...
    if sys.argv[1] == 'second_verify_pass':
        request_util = user_login(sys.argv[2])
        for user_id in sys.argv[3:]:
            helper.log('将user_id为%s的用户通过二审' % user_id)
    ...
    if sys.argv[1].startswith('grant_coupon'):
        ...
        if sys.argv[1].endswith('cleanup'):
            ...
            coupon_sent_user_keys = get_user_keys_from_system_grant_coupon()
